...
namespace StreamingServer
{
    ...
    {
        //Snapshot size
        private static int weight = 256;
        private static int height = 256;

        private List<Socket> _Clients;
        private Thread _Thread;

        public Server() : this(Capture.Snapshots(weight, height, true))
        {

        }

        public Server(IEnumerable<Image> imagesSource)
        {
        ...
        private void ClientThread(object client)
        {
            ...
            try
            {
                using (MjpegStream wr = new MjpegStream(new NetworkStream(socket, true)))
                {
                    ...
                    foreach (var imgStream in Capture.Streams(this.ImagesSource))
                    {
                        if (this.Interval > 0)
